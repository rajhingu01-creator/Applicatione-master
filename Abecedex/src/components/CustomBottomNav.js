...
import { StyleSheet, View, Text, TouchableOpacity, Platform, useWindowDimensions, Animated, PanResponder } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { Home, MessageSquare, Plus, Clapperboard, User } from 'lucide-react-native';
...
const CustomBottomNav = ({ navigation, activeRoute, isDark = false }) => {
  ...
  const toggleMenu = () => {
    setIsExpanded(prev => {
      ...
      if (nextValue) {
        ...
        // bottom: 40, so current position from top is approximately:
        const absoluteTop = height - 40 - (58 + 20) + currentY;
        if (absoluteTop < 350) {
  ...
  const navItems = [
    ...
    { name: 'Chat', icon: MessageSquare, route: 'ChatList' },
  ...
  const createSize = 58;
  const fullHeight = 350; // Total height when expanded
  ...
  const containerBorderColor = expandAnim.interpolate({
    inputRange: [0, 0.2, 1],
    outputRange: ['transparent', 'rgba(255,255,255,0.1)', 'rgba(255,255,255,0.1)']
  });
    ...
    >
      <Animated.View style={[
        ...
        { 
          ...
          flexDirection: expandDirection === 'up' ? 'column-reverse' : 'column',
        }
      ]}>
