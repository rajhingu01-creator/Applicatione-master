...
const FollowersScreen = ({ navigation, route }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (followersJson.success) {
        ...
        setFollowRequests(followersJson.data.requests || []);
      }
  ...
  const handleFollowRequest = async (userId, action) => {
    try {
      const token = await AsyncStorage.getItem("userToken");
      await fetch(`${API_URL}/profile/follow-requests/${userId}/${action}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchData();
  ...
  const renderUserItem = ({ item }) => (
    <View style={styles.userItem}>
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate("UserProfile", { userId: item.id, user: item })
        }
      >
      ...
      {item.requested ? (
        <View style={styles.requestActions}>
          ...
          <TouchableOpacity
            ...
            onPress={() => handleFollowRequest(item.id, "reject")}
          >
